...
export default function PublicationsSection() {
  ...
  // Auto-expand archive when user actively searches, filters, or sorts
  const isFiltering = searchQuery.trim() !== '' || selectedYear !== 'ALL' || sortBy !== 'newest';
  const showFullArchive = isExpanded || isFiltering;
  ...
  // Display 3 featured items in compact mode, or full paginated list when expanded
  const displayedPublications = useMemo(() => {
    if (!showFullArchive) {
      return filteredAndSortedPublications.slice(0, PREVIEW_LIMIT);
    }
    return paginatedPublications;
  }, [showFullArchive, filteredAndSortedPublications, paginatedPublications]);
  ...
  return (
    <section id="publications" className="mt-12 sm:mt-16 scroll-mt-24 px-3 sm:px-6 lg:px-8 min-w-0 max-w-full">
      ...
      <div className="border-b border-slate-200 pb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl sm:text-3xl font-black text-slate-900 lg:text-4xl">
              {showFullArchive ? t.publications.title : t.publications.recentTitle}
            </h2>
          ...
          <div className="flex items-center gap-3">
            ...
            {!showFullArchive && (
              <button
                ...
                className="hidden sm:inline-flex items-center gap-2 rounded-2xl border border-red-200 bg-white px-4 py-2.5 text-xs font-bold text-red-800 shadow-2xs transition hover:border-red-400 hover:bg-red-50 cursor-pointer"
              >
                <span>{t.publications.viewMore}</span>
                <svg className="h-3.5 w-3.5 text-red-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Filter and Search Bar (shown when full archive is opened) */}
        {showFullArchive && (
          <div id="publications-filter-bar">
  ...
  <div className="mt-8 space-y-6">
    {displayedPublications.length === 0 ? (
      ...
      {/* View More / Expand Archive Banner (shown in compact preview mode) */}
      {!showFullArchive && (
        <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-6 sm:p-8 text-center shadow-xs transition hover:border-red-200 hover:shadow-md">
          <div className="mx-auto max-w-xl">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-3 py-1 text-xs font-bold text-red-800 border border-red-200/60 mb-3">
              <span>{publications.length} {t.publications.itemsCount}</span>
            </div>
            <h3 className="text-lg sm:text-xl font-black text-slate-900">
              {lang === 'tr' ? 'Tüm Araştırma Arşivini Keşfedin' : 'Explore Full Research Archive'}
            </h3>
            <p className="mt-2 text-xs sm:text-sm text-slate-600 leading-relaxed">
              {t.publications.exploreArchiveDesc}
            </p>
            <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
              <button
                ...
                onClick={() => {
                  setIsExpanded(true);
                  setTimeout(() => {
                    const filterBar = document.getElementById('publications-filter-bar');
                    if (filterBar) filterBar.scrollIntoView({ behavior: 'smooth', block: 'start' });
                  }, 50);
                }}
                className="inline-flex items-center gap-2 rounded-full bg-red-700 px-6 py-3 text-xs sm:text-sm font-bold text-white shadow-md shadow-red-950/20 transition hover:bg-red-600 cursor-pointer"
              >
                <span>{t.publications.viewMore} ({publications.length})</span>
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
      ...
      {/* Numbered Pagination Controls (shown in full archive mode) */}
      {showFullArchive && totalPages > 1 && (
      ...
      {/* Show Less Button (shown when expanded in full archive mode) */}
      {showFullArchive && (
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-6 py-2.5 text-xs sm:text-sm font-bold text-slate-700 shadow-2xs transition hover:border-red-300 hover:text-red-700 hover:bg-slate-50 cursor-pointer"
          >
            <span>{t.publications.showLess}</span>
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
